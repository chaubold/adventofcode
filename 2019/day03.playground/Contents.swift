...
struct Instruction
{
  ...
  init(str: Substring) {
    ...
    dist = Int(str.dropFirst())!
  }
}

typealias Path = [Instruction]

let inputLines = input.split(separator: "\n")
let pathsElements = inputLines.map({$0.split(separator: ",")})
var pathsInstructions = [Path]()
for pathElements in pathsElements {
  pathsInstructions.append(pathElements.map( { Instruction(str: $0) } ))
}

struct Coord : Hashable {
  let x, y: Int
}
typealias PathIndex = Int

var canvas = [Coord:PathIndex]()

/** Returns closest intersection coord if any */
func draw(path: Path, index: PathIndex, onto canvas:[Coord:PathIndex]) -> Coord?
{
  var penPosition = Coord(x: 0, y: 0)
